  ...
  static queryStrategy(feed) {
    const strategy = {
      ...
      market_analysis: {
        ...
        total_loss_usd: feed.onchain_intelligence.exploit_summary.total_drained_usd,
        panic_level: feed.social_intelligence.panic_level,
        arbitrage_spreads: feed.market_intelligence.arbitrage_opportunities.map((opp) => ({
          asset: opp.asset,
      ...
      ai_decision: {
        approach: "Deploy simultaneous buy orders across CRV, alETH, and pETH. First fill cancels others.",
        reasoning: [
...
var fetchAggregatedFeed = (sendRequester, url) => {
  ...
  if (response.statusCode !== 200) {
    throw new Error(`Failed to fetch aggregated feed: HTTP ${response.statusCode}`);
  }
...
var executeCrisisResponse = (runtime2) => {
  ...
  runtime2.log(`
Social Intelligence:`);
  runtime2.log(`- Aggregate Sentiment: ${feed.social_intelligence.aggregate_sentiment}`);
  runtime2.log(`- Panic Level: ${feed.social_intelligence.panic_level * 100}%`);
  runtime2.log(`- Alerts: ${feed.social_intelligence.twitter_alerts.length} tweets from verified accounts`);
  feed.social_intelligence.twitter_alerts.slice(0, 2).forEach((alert) => {
    runtime2.log(`  - ${alert.account}: "${alert.message.substring(0, 80)}..."`);
  });
  runtime2.log(`
On-Chain Intelligence:`);
  runtime2.log(`- Total Drained: $${feed.onchain_intelligence.exploit_summary.total_drained_usd.toLocaleString()}`);
  runtime2.log(`- Affected Pools: ${feed.onchain_intelligence.exploit_summary.affected_pools.join(", ")}`);
  runtime2.log(`- Attack Pattern: ${feed.onchain_intelligence.exploit_summary.attack_pattern}`);
  runtime2.log(`- Confidence: ${feed.onchain_intelligence.exploit_summary.exploit_confidence * 100}%`);
  runtime2.log(`
Arbitrage Opportunities:`);
  feed.market_intelligence.arbitrage_opportunities.slice(0, 3).forEach((opp) => {
    const spreadInfo = opp.spread_percentage ? `${opp.spread_percentage}% spread` : `${opp.depeg_percentage}% depeg`;
    runtime2.log(`- ${opp.asset}: ${spreadInfo} | Risk: ${opp.risk_level} | Action: ${opp.recommended_action}`);
  });
  runtime2.log(`
Market Conditions:`);
  runtime2.log(`- Volatility: ${feed.market_intelligence.volatility.level}`);
  runtime2.log(`- Panic Selling: ${feed.market_intelligence.volatility.panic_selling}`);
  runtime2.log(`- Liquidity Crisis: ${feed.market_intelligence.volatility.liquidity_crisis}`);
  runtime2.log(`- Gas Price: ${feed.market_intelligence.gas_conditions.current_gwei} gwei`);
  runtime2.log(`
...
AI Strategy: ${strategy.strategy_type}`);
  runtime2.log(`Approach: ${strategy.ai_decision.approach}`);
  runtime2.log(`
AI Reasoning:`);
  strategy.ai_decision.reasoning.forEach((reason, idx) => {
    runtime2.log(`${idx + 1}. ${reason}`);
  });
  runtime2.log(`
...
Generated SwapVM Strategies:`);
  strategy.swapvm_strategies.forEach((strat, idx) => {
    runtime2.log(`${idx + 1}. [Priority ${strat.priority}] ${strat.action} ${strat.asset}`);
    runtime2.log(`   - Entry Price: $${strat.entry_price}`);
    runtime2.log(`   - Curve Type: ${strat.curve_type}`);
    runtime2.log(`   - Allocation: ${strat.allocation_percent}%`);
    runtime2.log(`   - Bytecode: ${strat.bytecode.substring(0, 80)}...`);
  });
  runtime2.log(`
Execution Plan:`);
  runtime2.log(`- Trigger: ${strategy.execution_plan.trigger}`);
